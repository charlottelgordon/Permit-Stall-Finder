...
from permit_stall_finder.orchestration.pipeline import (  # noqa: E402
    AnalysisOutcome,
    PipelineStage,
    run_pipeline,
)
...
def test_run_batch_runs_pipeline_once_per_permit_and_sorts_worst_first(monkeypatch):
    calls: list[str] = []

    def fake_run_pipeline(conn, permit_number, sample_size=None):
        calls.append(permit_number)
        outcome = (
            AnalysisOutcome.STALL_DETECTED if permit_number == "B" else AnalysisOutcome.NO_MATERIAL_STALL_DETECTED
        )
        return SimpleNamespace(
            ...
            stall_assessment=SimpleNamespace(detections=[]),
            developer_explanations=SimpleNamespace(explanations=[]),
        )

    monkeypatch.setattr(portfolio, "run_pipeline", fake_run_pipeline)
    monkeypatch.setattr(portfolio, "outcome_headline", lambda result: "headline")

    batch = portfolio.run_batch(conn=None, permit_numbers=["A", "B"], sample_size=10, progress=False)

    assert calls == ["A", "B"]
    assert [r.permit_number for r in batch.rows] == ["B", "A"]  # worst (stall detected) first
    assert set(batch.results_by_permit) == {"A", "B"}
    assert batch.errors == []


def test_run_batch_collects_errors_without_raising(monkeypatch):
    def fake_run_pipeline(conn, permit_number, sample_size=None):
        if permit_number == "BAD":
            raise portfolio.PipelineExecutionError(
                permit_number, PipelineStage.JOURNEY_RECONSTRUCTION, RuntimeError("simulated failure")
            )
        return SimpleNamespace(
            permit_number=permit_number,
            outcome=AnalysisOutcome.NO_MATERIAL_STALL_DETECTED,
            journey=SimpleNamespace(latest_snapshot=None, derived=None),
            stall_assessment=SimpleNamespace(detections=[]),
            developer_explanations=SimpleNamespace(explanations=[]),
        )
    ...
    batch = portfolio.run_batch(conn=None, permit_numbers=["OK", "BAD"], sample_size=10, progress=False)

    assert [r.permit_number for r in batch.rows] == ["OK"]
    assert [permit_number for permit_number, _ in batch.errors] == ["BAD"]
