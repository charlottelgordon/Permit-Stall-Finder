"""Portfolio triage view -- batch analysis across multiple permits, for
personas (general contractor, permit expediter) who need to scan many
active permits at once rather than read one closely.
...
def render(conn, *, sample_size: int) -> None:
    st.subheader("Portfolio triage")
    st.caption(
        "Scan multiple permits at once, worst first. Paste one permit number per line "
        "(or comma-separated)."
    )
    ...
    run_clicked = st.button("Analyze portfolio", type="primary")

    if run_clicked:
        permit_numbers = parse_permit_numbers(raw_text)
        if not permit_numbers:
            ...
            for i, permit_number in enumerate(permit_numbers):
                ...
                progress.progress((i + 1) / len(permit_numbers))
            progress.empty()

            if errors:
                st.warning(
                    f"{len(errors)} permit(s) couldn't be analyzed right now (the city's open "
                    "data service may be temporarily unavailable) and are omitted below: "
                    + ", ".join(p for p, _ in errors)
                )

            st.session_state.portfolio_rows = sort_rows(rows)
            st.session_state.portfolio_results = results_by_permit
